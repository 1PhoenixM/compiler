...
class ASTLeafNode extends ASTNode{
    constructor(public nodeName, public nodeVal, public nodeType, public lineNumber, public isUsed, public parent, public children, public visited, public scope){
		super(nodeName, parent, children, visited);
		this.nodeVal = nodeVal;
	...
	if (root !== null && root.children.length > 0) {
		...
		for(var i = 0; i < root.children.length; i++){
			...
			if(typeof ASTNodes[root.children[i].nodeName] !== "undefined"){
				...
				//Types are traversed after the fact, this sets types to values
...
function joinString(root: CSTNode, stringVal: string){
...
function scopeAndTypeCheck(root: ASTNode){
	...
	//Add new variables to the scope, and check for redeclaration of identifiers
		...
		//If not, add the new variable and its type to the current scope.
		else{
	...
	//Check If and While for scope and type
	...
	//Check EqCompareTest for scope and type
		...
		//Check if var exists in current scope in symbol table - and parent scope, if not
		if(root.children[0].nodeName === "LeftVal"){ //also rightval
			isFound = currentScope.find(root.children[0]);
		}
		...
		//scope should be checked when used as exp
	}
	else {
		//continues to traverse whole AST
	}
}
...
function writeCodes(root: ASTNode){
	...
	//If Output...
		...
		//If no variable found, printing a literal.
		else{
